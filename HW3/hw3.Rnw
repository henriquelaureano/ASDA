...
\textbf{Among those who do not support the restriction, what is the proportion of
        DJT sup- porters? What is the odds of selecting a participant who support
        DJT?} \\
...
\textbf{(i.) the the null and alternative hypotheses.} \\
...
\textbf{Assume that children’s score in the KidScore data set has a
        ...
        high school, and \(N(\mu_{2}, \sigma^{2}\) if the mother has not graduated
        from high school. Using the random sample collected in the KidScore data
        set,} \\
