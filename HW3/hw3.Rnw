...
bell shape behavior, and, in red we have the 2.5th and 97.5th percentiles, and in
both cases this percentiles are very close to -2, 2. Which is a sign that the
reference distributions are very similar to a Standard Normal distribution,
because we know that in a Standard Normal we have 95\% of the data between \(\pm\)
1.96 (very close to 2).
