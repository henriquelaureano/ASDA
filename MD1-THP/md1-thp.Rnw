...
\begin{minipage}{.4\textwidth}
 ...
 this values we can see that exist a difference, but that the difference is small
 and maybe not sufficient to be significatily.
...
<<echo=FALSE>>=
# <code r> ===================================================================== #
bx_rd.li <- mean(dataY[ , 1])
bx_rd.hi <- mean(dataY[ , 2])
bx_ld.li <- mean(dataY[ , 3])
bx_ld.hi <- mean(dataY[ , 4])

s_4 <-
  50*(mean(dataY[ , 1])-mean(dataY))**2 +
  50*(mean(dataY[ , 2])-mean(dataY))**2 +
  50*(mean(dataY[ , 3])-mean(dataY))**2 +
  50*(mean(dataY[ , 4])-mean(dataY))**2

s_p.2 <-
  .25*sum((dataY[ , 1]-mean(dataY[ , 1]))**2)/49 +
  .25*sum((dataY[ , 2]-mean(dataY[ , 2]))**2)/49 +
  .25*sum((dataY[ , 3]-mean(dataY[ , 3]))**2)/49 +
  .25*sum((dataY[ , 4]-mean(dataY[ , 4]))**2)/49
...
\begin{align*}
 ...
 \Sexpr{round(sum((dataY[ , 1]-mean(dataY[ , 1]))**2)/49, 4)} \quad
 & S_{{\rm RD-HI}}^{2} =
 \sum_{i=1}^{50} \frac{(X_{i}^{{\rm RD-HI}} - \bar{X}_{{\rm RD-HI}})^{2}}{50-1} =
 \Sexpr{round(sum((dataY[ , 2]-mean(dataY[ , 2]))**2)/49, 4)} \\
 S_{{\rm LD-LI}}^{2} & =
 \sum_{i=1}^{50} \frac{(X_{i}^{{\rm LD-LI}} - \bar{X}_{{\rm LD-LI}})^{2}}{50-1} =
 \Sexpr{round(sum((dataY[ , 3]-mean(dataY[ , 3]))**2)/49, 4)} \quad
 & S_{{\rm LD-HI}}^{2} =
 \sum_{i=1}^{50} \frac{(X_{i}^{{\rm LD-HI}} - \bar{X}_{{\rm LD-HI}})^{2}}{50-1} =
 \Sexpr{round(sum((dataY[ , 4]-mean(dataY[ , 4]))**2)/49, 4)}
\end{align*}
\begin{align*}
...
\fbox{\begin{minipage}{\textwidth}
       ...
       evidence that at least one mean reduction is different from the others.
      \end{minipage}}
