...
\begin{minipage}{12cm}
 \[ T_{1, {\rm obs}} = \frac{0.7 - 0.6}{\sqrt{
    ...
    \frac{0.1}{\Sexpr{.7*.3/100 + .6*.4/500}},
 \]
\end{minipage}
\begin{minipage}{1cm}
 \[ \boxed{T_{1, {\rm obs}} = \Sexpr{.1/(.7*.3/100 + .6*.4/500)}} \]
\end{minipage}
...
<<fig.width=10, fig.cap="Histogram of the reference distribution (1000 relications).">>=
# <code r> ===================================================================== #
# setting sample sizes
n_a <- 100 ; n_b <- 500
# setting probabilities
p_a <- .7 ; p_b <- .6
# creating empth vector to store the proportion difference
diffs <- numeric(1000)
# number of replications
B <- 1e3

# creating the reference distribution
for (i in 1:B){
  # sampling
  samp_a <- rbinom(1, n_a, p_a)
  samp_b <- rbinom(1, n_b, p_b)
  # calculating the proportions
  prop_a <- mean(samp_a)
  prop_b <- mean(samp_b)
  # calculating the proportion difference
  diff_ab <- prop_a - prop_b
  # storing the proportion difference
  diffs[i] <- diff_ab
}
# histogram
hist(diffs
...
hist(diffs
     ...
     , col = "#0080ff"
     , border = "orange")
abline(v = quantile(diffs, .95), col = 2, lwd = 2)
# </code r> ==================================================================== #
@
...
\textbf{Construct the table of (estimated) \underline{expected} counts of patients
        with significant drop under the null hypothesis \(H_{0}\). Construct the
        table of \underline{observed} counts.}\\
...
\textbf{To conduct this test you need to (i.) compute the observed test statistic
        \(T_{2, obs}\); (ii.) state the reference distribution of \(T_{2}\);
        (iii.) identify the acceptance and rejection regions for a given 
        probability of type I error; (iv.) decision and conclusion.} \\
