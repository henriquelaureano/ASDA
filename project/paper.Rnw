...
\JournalInfo{STAT 210 - Applied Statistics and Data Analysis,
...
\noindent The terms interpretation can be done by odds ratio (OR).

\noindent In the context of generalized linear models for binary data, the logit is the
...
\begin{table}[H]
 ...
 \caption{Summary of the final fitted logistic regression with the estimated
          ...
          and related p-values. \textbf{In bold}, for easy identification, are the
...
a bigger sensitivity (true positive rate - proportion of patients with signs of DR
...
Thinking in the correlation between this variables, a PCA analysis was performe,
...
ratio approach to zero, which means that for high confidence levels differences
...
\noindent The variable selection was made with the \texttt{stepAIC()} R function. All the
steps are presented in Table \ref{tab:app_aic}.

\noindent In the first step we see that the smallest AIC (with the AIC the lower the better)
